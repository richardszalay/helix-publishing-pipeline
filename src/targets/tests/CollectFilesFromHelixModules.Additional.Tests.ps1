...
Describe "CollectFilesFromHelixModules.Additional" {
    ...
    Context "when additional files are configured" {
        ...
        } -Name "CollectFilesFromHelixModulesAdditional" -TargetName "CollectHelixModules;CollectFilesFromHelixModulesAdditional" -OutputItem "FilesForPackagingFromHelixModules").DestinationRelativePath
        ...
        It "should include matching files added with TargetPath" {
            $result -contains "App_Data\unicorn\HelixBuild\Feature\1\sub\Content1.yml" | Should Be $true
        }
    }
